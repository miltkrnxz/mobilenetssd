...
lineaccesstoken = 'NRxuD82pF/rAiZ74N2OIPF0hbJ/3i+dkb3PJGkNEtxaMDt0Yjg/bh+VEHNvoioxLEkziOslCWhzkI7F5JAx2NINVEUNH16YU7wU3YpZWwto97WPa8ax5sEdxKRoz3DdY3Y9GvrlE4QWJn1gRdijBRAdB04t89/1O/w1cDnyilFU='
...
def event_handle(event):
    ...
    if msgType == "text":
        ...
        if (msg == "สวัสดี") :
            replyObj = TextSendMessage(text="จย้าา..ดีจ้าา")
            line_bot_api.reply_message(rtoken, replyObj)
        elif (msg == "ทำการบ้านยัง") :
            replyObj = TextSendMessage(text="ยังอ่ะขี้เกียจ")
            line_bot_api.reply_message(rtoken, replyObj)
        elif (msg == "ขอตังค์หน่อยเด้") :
            replyObj = TextSendMessage(text="ไม่มีเช่นกันค่ะ")
            line_bot_api.reply_message(rtoken, replyObj)
        else :  
            headers = request.headers
            json_headers = ({k:v for k, v in headers.items()})
            json_headers.update({'Host':'https://dialogflow.cloud.google.com'})
            url = "https://dialogflow.cloud.google.com/v1/integrations/line/webhook/791a0343-6daf-4197-be68-5ce11a6cb5d5"
